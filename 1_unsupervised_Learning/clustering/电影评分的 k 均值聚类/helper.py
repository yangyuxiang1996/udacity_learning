...
def draw_movie_clusters(clustered, max_users, max_movies):
    ...
    for cluster_id in clustered.group.unique():
        ...
        d = d.reindex_axis(d.mean().sort_values(ascending=False).index, axis=1) # 按照列从大到小排序
        d = d.reindex_axis(d.count(axis=1).sort_values(ascending=False).index) # 按照行从大到小排序
...
def draw_movies_heatmap(most_rated_movies_users_selection, axis_labels=True):
    ...
    if axis_labels:
        ax.set_yticks(np.arange(most_rated_movies_users_selection.shape[0]) , minor=False) # 设定y轴
        ax.set_xticks(np.arange(most_rated_movies_users_selection.shape[1]) , minor=False) # 设定x轴
        ax.invert_yaxis() # 翻转y轴
        ax.xaxis.tick_top() # 将x轴移至顶部
        labels = most_rated_movies_users_selection.columns.str[:40]
        ax.set_xticklabels(labels, minor=False) # 设定x轴标签
        ax.set_yticklabels(most_rated_movies_users_selection.index, minor=False) # 设定y轴标签
        plt.setp(ax.get_xticklabels(), rotation=90) # x轴标签旋转90度
    else:
